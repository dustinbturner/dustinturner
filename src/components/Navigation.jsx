...
function TopLevelNavItem({ href, children }) {
  return (
    <li className="md:hidden">
      <Link
        href={href}
        className="block py-1 text-sm transition text-neutral-600 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white"
      >
        {children}
      </Link>
    </li>
...
function ActivePageMarker({ group, pathname }) {
  ...
  return (
    <motion.div
      layout
      className="absolute w-px h-6 bg-green-500 left-2"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1, transition: { delay: 0.2 } }}
      exit={{ opacity: 0 }}
      style={{ top }}
  ...
  return (
    <li className={clsx('relative mt-6', className)}>
      ...
      <div className="relative pl-2 mt-3">
        ...
        <motion.div
          layout
          className="absolute inset-y-0 w-px left-2 bg-neutral-900/10 dark:bg-white/5"
        />
